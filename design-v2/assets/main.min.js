...
try { if (localStorage.getItem('fs-review') === REVIEW_DIGEST) return; } catch (e) {  }
...
try { localStorage.setItem('fs-review', REVIEW_DIGEST); } catch (e2) {  }
...
lbIndeks = (lbIndeks + delta + lbLista.length) % lbLista.length;
lbPokaz();
}
function lbOpen(img) {
...
if (c.v !== CONSENT_VERSION || !c.ts) return false;
